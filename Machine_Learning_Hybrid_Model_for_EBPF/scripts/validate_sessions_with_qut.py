...
def derive_label(record: Dict, filename: str, mode: str) -> int | None:
    """Derive label from JSON label field or name-based heuristic."""
...
def load_session_features(session_dirs: List[Path], label_mode: str) -> Tuple[np.ndarray, np.ndarray, List[Dict]]:
    ...
    for session_dir in session_dirs:
        ...
        for path in sorted(session_dir.glob("*.json")):
            ...
            label = derive_label(record, path.name, label_mode)
            if label is None:
                skipped += 1
                continue
    ...
    if skipped > 0:
        if label_mode == "json":
            ...
            print(f"WARN: skipped {skipped} sessions without usable label")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="json",
        choices=["json", "heuristic"],
        help="How to derive labels when validating sessions",
    )
    ...
    if args.sessions_dir:
        ...
        default_dir = PROJECT_ROOT / "sessions"
        session_dirs.append(default_dir)
        holdout_dir = PROJECT_ROOT / "sessions_holdout"
    ...
    x_val_ebpf, y_val_ebpf, meta = load_session_features(session_dirs, args.label_mode)
